...
class Workflow:
    ...
    def addTask(self, task: Task.Task | int):
        if (self.hasTask(task)):
            raise Exception(f"Task ID {task.taskID if isinstance(task, Task.Task) else task} already exists in workflow {self.WORKFLOW_ID}.")
        else:
            if isinstance(task, int):
                task = Task.Task(self.WORKFLOW_ID, task, None)
            self._tasks.append(task)
    



    def hasTask(self, task: Task.Task | int):
