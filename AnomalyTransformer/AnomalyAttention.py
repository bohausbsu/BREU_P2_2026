...
class AnomalyTransformer(nn.Module):
    ...
    def minimize_loss(x, x_hat, P_list, S_list, lam):
        ...
        S_detach = [S.detach() for S in S_list]
        disc = AnomalyAttention.association_discrepancy(P_list, S_detach).mean()
        return recon + lam * disc
    @staticmethod
    def maximize_loss(x, x_hat, P_list, S_list, lam):
        recon = ((x - x_hat) ** 2).mean()
        P_detach = [P.detach() for P in P_list]
        disc = AnomalyAttention.association_discrepancy(P_detach, S_list).mean()
        return recon - lam * disc
    @staticmethod
    def anomaly_score(x, x_hat, P_list, S_list):
        disc = AnomalyAttention.association_discrepancy(P_list, S_list)
        recon_err = ((x - x_hat) ** 2).mean(dim=-1)
        score = torch.softmax(-disc, dim=-1) * recon_err
        return score
