...
def combine_and_average(results_dir, pattern, out_csv, out_png, title):
    ...
    combined = combined.query('model_class != "ALL"')
    ...
    grouped = (
        combined.groupby("model_class")[["precision", "recall", "f1", "total_time_sec"]]
        .mean()
        .reset_index()
    )
    ...
    out_df = grouped[["model_class", "size_label", "precision", "recall", "f1", "total_time_sec"]].copy()
    ...
    total_time = grouped["total_time_sec"].tolist()

    x = list(range(len(labels)))

    fig, ax = plt.subplots(figsize=(9, 5))
    ax.plot(x, total_time, "k-o", label="total_runtime")
    ax.set_xticks(x)
